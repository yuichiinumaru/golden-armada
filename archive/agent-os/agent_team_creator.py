#!/usr/bin/env python3
"""
AGENT TEAM CREATOR - Script para criar um Team com todos os agentes

Este script carrega todos os agentes da pasta 01-generated-agents e os organiza
em um Team do Agno para trabalho colaborativo.
"""
...
logger = logging.getLogger("agent_team_creator")

def setup_logging(verbose: bool = False):
    """Configura logging para o Agent Team Creator."""
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("agent_team_creator.log", encoding="utf-8")
        ]
    )
...
def create_agent_team(agents: List[Any], team_name: str = None, description: str = None) -> Team:
    """
    Cria um Team com os agentes carregados.
    ...
    Returns:
        Instância do Team configurada
    """
    if team_name is None:
        team_name = f"AgentMaker Team - {len(agents)} Agentes"
    
    if description is None:
        description = f"Team colaborativo com {len(agents)} agentes especializados do AgentMaker"
    
    logger.info(f"👥 Criando Team '{team_name}' com {len(agents)} agentes")
    
    # Criar o team diretamente com os agentes
    team = Team(
        name=team_name,
        description=description,
        members=agents,
        instructions=[
            f"Você é o {team_name}. Trabalhe em conjunto com os outros agentes especializados.",
            "Distribua tarefas entre os agentes baseado em suas especialidades.",
            "Garanta que todos os agentes trabalhem em harmonia e compartilhem conhecimento.",
            "Use a expertise de cada agente para resolver problemas complexos de forma colaborativa."
        ],
    ...
    logger.info("✅ Team criado com sucesso")
    return team

def create_agent_os_with_team(team: Team, description: str = None) -> AgentOS:
    """
    Cria uma instância do Agent OS com o team.
    ...
    """
    if description is None:
        description = f"AgentMaker OS com Team - {team.name}"
    
    logger.info(f"🏗️  Criando Agent OS com Team: {team.name}")
...
async def start_server(agent_os: AgentOS, port: int = 8000, verbose: bool = False):
    ...
    logger.info(f"👥 Teams: http://localhost:{port}/teams")
    ...
    try:
        ...
        if verbose:
            ...
            important_routes = ["/docs", "/teams", "/agents", "/config", "/openapi.json"]
...
async def main():
    parser = argparse.ArgumentParser(description="Agent Team Creator - Criar Team com todos os agentes")
    ...
    logger.info("🚀 AGENT TEAM CREATOR - Iniciando...")
    ...
    try:
        ...
        # Criar team
        team = create_agent_team(agents, args.team_name, args.description)
        
        # Criar Agent OS com team
        agent_os = create_agent_os_with_team(team, args.description)
        
        # Iniciar servidor
        await start_server(agent_os, args.port, args.verbose)
        
        logger.info("✅ Agent Team Creator executado com sucesso!")
        return 0
