...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("location_scout_sysp.json")

logger = logging.getLogger("LocationScout")
...
def load_system_prompt(path: Path) -> Dict[str, Any]:
    ...
    default_prompt: Dict[str, Any] = {
        "description": "Expert Location Scout and Geographic Analyst.",
        "instructions": [
...
class LocationScoutAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert Location Scout.")
    ...
    async def run_task(self, task: str) -> Dict[str, Any]:
        ...
        logger.info(f"🚀 Starting task for LocationScout...")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert Location Scout.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="LocationScout", verbose=args.verbose)
