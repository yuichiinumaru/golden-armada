...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("seo_keyword_strategist_sysp.json")

logger = logging.getLogger("SeoKeywordStrategist")
...
class SeoKeywordStrategistAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Analyzes keyword usage in provided content, calculates density, suggests semantic variations and LSI keywords based on the topic. Prevents over-optimization. Use PROACTIVELY for content optimization.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for SeoKeywordStrategist...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Analyzes keyword usage in provided content, calculates density, suggests semantic variations and LSI keywords based on the topic. Prevents over-optimization. Use PROACTIVELY for content optimization.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="SeoKeywordStrategist", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = SeoKeywordStrategistAgent()
    ...
    report_header = f"""
# Agent Report: SeoKeywordStrategist
