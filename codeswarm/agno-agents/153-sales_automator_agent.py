...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("sales_automator_sysp.json")

logger = logging.getLogger("SalesAutomator")
...
class SalesAutomatorAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Draft cold emails, follow-ups, and proposal templates. Creates pricing pages, case studies, and sales scripts. Use PROACTIVELY for sales outreach or lead nurturing.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for SalesAutomator...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Draft cold emails, follow-ups, and proposal templates. Creates pricing pages, case studies, and sales scripts. Use PROACTIVELY for sales outreach or lead nurturing.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="SalesAutomator", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = SalesAutomatorAgent()
