...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("laravel_eloquent_expert_sysp.json")

logger = logging.getLogger("LaravelEloquentExpert")
...
class LaravelEloquentExpertAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for LaravelEloquentExpert...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Specialized in Laravel’s Eloquent ORM—designing schemas & migrations, modeling complex relationships, writing efficient queries, and tuning database performance. **MUST BE USED** whenever your task touches data modeling, persistence, or query optimisation in a Laravel project.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="LaravelEloquentExpert", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = LaravelEloquentExpertAgent()
    ...
    report_header = f"""
# Agent Report: LaravelEloquentExpert
