...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("refactoring_specialist_sysp.json")

logger = logging.getLogger("RefactoringSpecialist")
...
class RefactoringSpecialistAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for RefactoringSpecialist...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert refactoring specialist mastering safe code transformation techniques and design pattern application. Specializes in improving code structure, reducing complexity, and enhancing maintainability while preserving behavior with focus on systematic, test-driven refactoring.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="RefactoringSpecialist", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = RefactoringSpecialistAgent()
    ...
    report_header = f"""
# Agent Report: RefactoringSpecialist
