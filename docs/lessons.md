...
*   **Model Compatibility (ADK 1.1.1):**
    *   `gemini-2.5-flash-preview-05-20` was found to be a working and compatible model with the Google ADK 1.1.1 setup used.
...
*   **Gemini Model Naming:** Be specific with model names (e.g., `gemini-2.5-flash-preview-05-20` vs. `gemini-pro`). The ADK 1.1.1 seemed to default to a `v1beta` API that had limited model availability, causing `404` errors for some valid current model names.
