...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("code_archaeologist_sysp.json")
...
class CodeArchaeologistAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for CodeArchaeologist...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="MUST BE USED to explore and document unfamiliar, legacy, or complex codebases. Use PROACTIVELY before refactors, onboarding, audits, or risk reviews. Produces a full-length report—architecture, metrics, risks, and a prioritised action plan—that other sub-agents can act on.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="CodeArchaeologist", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = CodeArchaeologistAgent()
    ...
    report_header = f"""
# Agent Report: CodeArchaeologist
