...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("visual_storyteller_sysp.json")

logger = logging.getLogger("VisualStoryteller")
...
class VisualStorytellerAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for VisualStoryteller...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Use this agent when creating visual narratives, designing infographics, building presentations, or communicating complex ideas through imagery. This agent specializes in transforming data and concepts into compelling visual stories that engage users and stakeholders. Examples:\n\n<example>\nContext: Creating app onboarding illustrations")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="VisualStoryteller", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = VisualStorytellerAgent()
    ...
    report_header = f"""
# Agent Report: VisualStoryteller
