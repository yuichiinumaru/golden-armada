...
except ImportError:  # pragma: no cover - optional dependency per agent
    cognee = None  # type: ignore[assignment]

from source.shared.cognee_rotation import (  # noqa: E402
    configure_cognee_with_rotation,
    get_cognee_with_rotation,
    initialize_cognee_rotation,
)

SYSTEM_PROMPT_FILE = Path(__file__).with_name("kubernetes_architect_sysp.json")

logger = logging.getLogger("KubernetesArchitect")
...
class KubernetesArchitectAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
        ...
        self.cognee_client: Optional[Any] = None
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert Kubernetes architect specializing in cloud-native infrastructure, advanced GitOps workflows (ArgoCD/Flux), and enterprise container orchestration. Masters EKS/AKS/GKE, service mesh (Istio/Linkerd), progressive delivery, multi-tenancy, and platform engineering. Handles security, observability, cost optimization, and developer experience. Use PROACTIVELY for K8s architecture, GitOps implementation, or cloud-native platform design.")
    ...
    async def initialize_cognee_memory(self) -> Optional[Any]:
        """Initialize Cognee memory with shared API key rotation support."""

        if cognee is None:
            logger.warning("Cognee package not installed; skipping memory initialization")
            return None

        rotation_enabled = initialize_cognee_rotation()

        if rotation_enabled:
            self.cognee_client = get_cognee_with_rotation()
            await _maybe_await(self.cognee_client.initialize())
            logger.info("Cognee initialized with rotation service")
        else:
            configure_cognee_with_rotation(cognee)
            await _maybe_await(cognee.cognify())
            await _maybe_await(cognee.memify())
            self.cognee_client = cognee
            logger.info("Cognee initialized with single API key")

        return self.cognee_client

    def get_cognee_client(self) -> Optional[Any]:
        """Returns initialized Cognee client (if available)."""

        if self.cognee_client is None and cognee is not None:
            logger.debug("Cognee client not initialized yet; call initialize_cognee_memory() first")
        return self.cognee_client
    
    async def run_task(self, task: str) -> str:
        prompt = self.build_prompt(task)
        
        print(f"🚀 Starting task for KubernetesArchitect...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert Kubernetes architect specializing in cloud-native infrastructure, advanced GitOps workflows (ArgoCD/Flux), and enterprise container orchestration. Masters EKS/AKS/GKE, service mesh (Istio/Linkerd), progressive delivery, multi-tenancy, and platform engineering. Handles security, observability, cost optimization, and developer experience. Use PROACTIVELY for K8s architecture, GitOps implementation, or cloud-native platform design.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="KubernetesArchitect", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = KubernetesArchitectAgent()
    ...
    report_header = f"""
# Agent Report: KubernetesArchitect
