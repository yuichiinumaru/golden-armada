...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("rails_activerecord_expert_sysp.json")

logger = logging.getLogger("RailsActiverecordExpert")
...
class RailsActiverecordExpertAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert in Rails ActiveRecord optimisation, complex queries, and database performance. Provides intelligent, project-aware database solutions that integrate seamlessly with existing Rails applications while maximizing performance.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for RailsActiverecordExpert...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert in Rails ActiveRecord optimisation, complex queries, and database performance. Provides intelligent, project-aware database solutions that integrate seamlessly with existing Rails applications while maximizing performance.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="RailsActiverecordExpert", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = RailsActiverecordExpertAgent()
    ...
    report_header = f"""
# Agent Report: RailsActiverecordExpert
