...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("django_api_developer_sysp.json")

logger = logging.getLogger("DjangoApiDeveloper")
...
class DjangoApiDeveloperAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert Django API developer specializing in Django REST Framework and GraphQL. Use explicitly for Django API development, DRF serializers, viewsets, or GraphQL schemas. Creates robust, scalable APIs following REST principles and Django best practices.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for DjangoApiDeveloper...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert Django API developer specializing in Django REST Framework and GraphQL. Use explicitly for Django API development, DRF serializers, viewsets, or GraphQL schemas. Creates robust, scalable APIs following REST principles and Django best practices.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="DjangoApiDeveloper", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = DjangoApiDeveloperAgent()
    ...
    report_header = f"""
# Agent Report: DjangoApiDeveloper
