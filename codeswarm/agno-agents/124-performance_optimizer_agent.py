...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("performance_optimizer_sysp.json")
...
class PerformanceOptimizerAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "MUST BE USED whenever users report slowness, high cloud costs, or scaling concerns. Use PROACTIVELY before traffic spikes. Identifies bottlenecks, profiles workloads, and applies optimisations for blazingly fast systems.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for PerformanceOptimizer...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="MUST BE USED whenever users report slowness, high cloud costs, or scaling concerns. Use PROACTIVELY before traffic spikes. Identifies bottlenecks, profiles workloads, and applies optimisations for blazingly fast systems.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="PerformanceOptimizer", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = PerformanceOptimizerAgent()
    ...
    report_header = f"""
# Agent Report: PerformanceOptimizer
