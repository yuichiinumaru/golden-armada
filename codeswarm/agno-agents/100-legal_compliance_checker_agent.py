...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("legal_compliance_checker_sysp.json")

logger = logging.getLogger("LegalComplianceChecker")
...
class LegalComplianceCheckerAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for LegalComplianceChecker...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Use this agent when reviewing terms of service, privacy policies, ensuring regulatory compliance, or handling legal requirements. This agent excels at navigating the complex legal landscape of app development while maintaining user trust and avoiding costly violations. Examples:\n\n<example>\nContext: Launching app in European markets")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="LegalComplianceChecker", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = LegalComplianceCheckerAgent()
    ...
    report_header = f"""
# Agent Report: LegalComplianceChecker
