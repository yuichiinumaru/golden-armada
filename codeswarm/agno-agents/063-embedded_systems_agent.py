...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("embedded_systems_sysp.json")

logger = logging.getLogger("EmbeddedSystems")
...
class EmbeddedSystemsAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for EmbeddedSystems...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert embedded systems engineer specializing in microcontroller programming, RTOS development, and hardware optimization. Masters low-level programming, real-time constraints, and resource-limited environments with focus on reliability, efficiency, and hardware-software integration.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="EmbeddedSystems", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = EmbeddedSystemsAgent()
