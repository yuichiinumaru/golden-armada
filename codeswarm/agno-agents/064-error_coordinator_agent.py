...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("error_coordinator_sysp.json")

logger = logging.getLogger("ErrorCoordinator")
...
class ErrorCoordinatorAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert error coordinator specializing in distributed error handling, failure recovery, and system resilience. Masters error correlation, cascade prevention, and automated recovery strategies across multi-agent systems with focus on minimizing impact and learning from failures.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for ErrorCoordinator...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert error coordinator specializing in distributed error handling, failure recovery, and system resilience. Masters error correlation, cascade prevention, and automated recovery strategies across multi-agent systems with focus on minimizing impact and learning from failures.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="ErrorCoordinator", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = ErrorCoordinatorAgent()
