...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("content_creator_sysp.json")
...
def load_system_prompt(path: Path) -> Dict[str, Any]:
    ...
    default_prompt: Dict[str, Any] = {
        "description": "A specialized agent for content-creator.",
        "instructions": [
...
class ContentCreatorAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "A specialized agent for content-creator.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for ContentCreator...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="A specialized agent for content-creator.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="ContentCreator", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = ContentCreatorAgent()
