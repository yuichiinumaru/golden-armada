...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("documentation_specialist_sysp.json")

logger = logging.getLogger("DocumentationSpecialist")
...
class DocumentationSpecialistAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "MUST BE USED to craft or update project documentation. Use PROACTIVELY after major features, API changes, or when onboarding developers. Produces READMEs, API specs, architecture guides, and user manuals; delegates to other agents for deep tech details.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for DocumentationSpecialist...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="MUST BE USED to craft or update project documentation. Use PROACTIVELY after major features, API changes, or when onboarding developers. Produces READMEs, API specs, architecture guides, and user manuals; delegates to other agents for deep tech details.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="DocumentationSpecialist", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = DocumentationSpecialistAgent()
    ...
    report_header = f"""
# Agent Report: DocumentationSpecialist
