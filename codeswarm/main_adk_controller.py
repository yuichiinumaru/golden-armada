...
async def execute_agent_and_get_result(
    current_agent_instance, 
    agent_input_dict, 
    agent_identifier_for_logging,
    session_service,
    session_id,
    user_id,
    debug_mode 
):
    if debug_mode: print(f"DEBUG: session_service instance at start of execute_agent_and_get_result: {id(session_service)}")
    ...
    if debug_mode: print(f"DEBUG: session_service instance in current_runner: {id(current_runner.session_service)}")

    agent_name = getattr(current_agent_instance, 'name', agent_identifier_for_logging)
    ...
    if agent_name.startswith("DevAgentADK") or agent_name == "AdminAgentADK" or agent_identifier_for_logging == "admin_logger":
        ...
        if tool_call_detected_in_input or tool_is_dangerous_itself:
            confirm_msg = f"[Segurança] O Agente '{agent_name}' (Task: {agent_input_dict.get('current_phase', agent_input_dict.get('task_id', 'N/A'))}) pode usar/solicitar uma ferramenta perigosa. Permitir? [y/N]: "
    ...
    if debug_mode: print(f"DEBUG: Chamando run_async para {agent_name} com user_id={user_id}, session_id={session_id}")
    ...
    try:
        async for event in current_runner.run_async(
            ...
            if debug_mode: print(", ".join(log_parts))
            
            if event.is_final_response():
                if debug_mode: print(f"DEBUG Final Event ({agent_name}): {event}")
                if event.content and event.content.parts:
...
async def run_admin_task_assignment_phase(admin_agent_instance, project_goal, target_project_path, current_round, previous_summaries, session_service, session_id, user_id, debug_mode):
    ...
    admin_task_assignment_result = await execute_agent_and_get_result(
        ...
        admin_task_input_dict,
        "admin_task_assigner",
        session_service, session_id, user_id, debug_mode
    )
    ...
    if admin_task_assignment_result and isinstance(admin_task_assignment_result, dict):
        try:
            ...
            if 'output_text' in admin_task_assignment_result and admin_task_assignment_result['output_text']:
...
async def process_task_with_dev_revisor_loop(task_details_dict: dict, round_context: dict, session_service, session_id, user_id) -> dict:
    ...
    for attempt_num in range(1, MAX_DEV_REVISOR_CYCLES + 1):
        attempt = attempt_num # Store current attempt for summary
        ...
        current_dev_task_input = {
            ...
            "target_project_path": target_project_path,
            "previous_revision_feedback": previous_revision_feedback
        }
        if debug_mode: print(f"[Orchestrator] DevAgent {dev_id} Input (Attempt {attempt}): {current_dev_task_input}")

        dev_result = await execute_agent_and_get_result(
            dev_agent_instance, current_dev_task_input, f"DevAgent_{dev_id}_Attempt_{attempt}",
            session_service, session_id, user_id, debug_mode
        )
        if debug_mode: print(f"[DevAgent {dev_id} Output (Attempt {attempt})] {dev_result}")

        if not dev_result or dev_result.get("status") != "success": 
        ...
        revisor_input_dict = {
            "file_to_review": absolute_file_path, 
            "focus_areas": task_details_dict.get("revisor_focus_areas", "Revisão geral"),
            "target_project_path": target_project_path,
            "dev_task_description": task_details_dict.get("dev_task_description")
        }
        if debug_mode: print(f"[Orchestrator] RevisorAgent {revisor_id} Input (Attempt {attempt}): {revisor_input_dict}")

        revisor_result = await execute_agent_and_get_result(
            revisor_agent_instance, revisor_input_dict, f"RevisorAgent_{revisor_id}_Attempt_{attempt}",
            session_service, session_id, user_id, debug_mode
        )
        if debug_mode: print(f"[RevisorAgent {revisor_id} Output (Attempt {attempt})] {revisor_result}")

        if not revisor_result or revisor_result.get("status") not in ["success", "requires_changes"]: 
        ...
        if isinstance(revisor_result, dict) and revisor_result.get("approved") == True: 
        ...
        if not previous_revision_feedback and not approved : 
    ...
    return {
        ...
        "task_details_original": task_details_dict, 
...
async def run_admin_logging_phase(admin_agent_instance, dev_outputs, revisor_feedback, target_project_path, current_round, session_service, session_id, user_id, debug_mode):
    admin_logging_input_dict = {
        "dev_outputs": dev_outputs, # Ensure this is a list of relevant dev outputs
        "revisor_feedback": revisor_feedback, # Ensure this is a list of relevant revisor feedback
        "target_project_path": target_project_path,
        "round": current_round, 
    ...
    admin_log_result = await execute_agent_and_get_result(
        admin_agent_instance, admin_logging_input_dict, "admin_logger",
        session_service, session_id, user_id, debug_mode
    )
...
def save_round_state(session, state_file_path, current_round_summary, target_project_path): 
...
async def execute_round(round_context: dict, session_service) -> dict: # Added session_service
    ...
    debug_mode = round_context["debug_mode"]
    session_id = round_context["session_id"] # Get session_id from context
    user_id = round_context["user_id"]       # Get user_id from context
    ...
    task_list = await run_admin_task_assignment_phase(
        ...
        session_service, session_id, user_id, debug_mode # Pass session_service, session_id, user_id
    )
    round_context["task_list"] = task_list 
    if not task_list:
    ...
    for task_detail in task_list:
        task_result_summary = await process_task_with_dev_revisor_loop(
            task_detail, round_context, 
            session_service, session_id, user_id # Pass session_service, session_id, user_id
        )
    ...
    admin_log_result = await run_admin_logging_phase(
        ...
        current_round_num, session_service, session_id, user_id, debug_mode # Pass session_service, session_id, user_id
    )
    round_context["admin_logging_result"] = admin_log_result 
...
async def main_async(args_list=None): 
    ...
    for current_round_num in range(1, args.rounds + 1):
        current_round_context = {
            ...
            "user_id": MAIN_USER_ID, # Added user_id
            ...
            "session_summaries": session.state.get("summaries", []) if isinstance(session.state, dict) else [],
            "debug_mode": args.debug
        }
        
        # Pass session_service to execute_round
        round_results_context = await execute_round(current_round_context, session_service)
        ...
        current_round_summary_data = {
            ...
            "tasks_assigned": round_results_context.get("task_list", []), 
            "dev_revisor_cycle_outputs": round_results_context.get("dev_revisor_cycle_outputs", []), 
