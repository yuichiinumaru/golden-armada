...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("senior_backend_architect_sysp.json")

logger = logging.getLogger("SeniorBackendArchitect")
...
class SeniorBackendArchitectAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Senior backend engineer and system architect with 10+ years at Google, leading multiple products with 10M+ users. Expert in Rust, Go and TypeScript, specialising in distributed systems, high performance APIs, and production grade infrastructure. Masters both technical implementation and system design with a track record of zero-downtime deployments and minimal production incidents.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for SeniorBackendArchitect...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Senior backend engineer and system architect with 10+ years at Google, leading multiple products with 10M+ users. Expert in Rust, Go and TypeScript, specialising in distributed systems, high performance APIs, and production grade infrastructure. Masters both technical implementation and system design with a track record of zero-downtime deployments and minimal production incidents.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="SeniorBackendArchitect", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = SeniorBackendArchitectAgent()
    ...
    report_header = f"""
# Agent Report: SeniorBackendArchitect
