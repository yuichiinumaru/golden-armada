...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("analytics_reporter_sysp.json")
...
class AnalyticsReporterAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for AnalyticsReporter...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Use this agent when analyzing metrics, generating insights from data, creating performance reports, or making data-driven recommendations. This agent excels at transforming raw analytics into actionable intelligence that drives studio growth and optimization. Examples:\n\n<example>\nContext: Monthly performance review needed")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="AnalyticsReporter", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = AnalyticsReporterAgent()
    ...
    report_header = f"""
# Agent Report: AnalyticsReporter
