...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("typescript_pro_sysp.json")

logger = logging.getLogger("TypescriptPro")
...
class TypescriptProAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert TypeScript developer specializing in advanced type system usage, full-stack development, and build optimization. Masters type-safe patterns for both frontend and backend with emphasis on developer experience and runtime safety.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for TypescriptPro...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert TypeScript developer specializing in advanced type system usage, full-stack development, and build optimization. Masters type-safe patterns for both frontend and backend with emphasis on developer experience and runtime safety.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="TypescriptPro", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = TypescriptProAgent()
