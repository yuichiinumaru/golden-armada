...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("workflow_optimizer_sysp.json")
...
class WorkflowOptimizerAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for WorkflowOptimizer...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Use this agent for optimizing human-agent collaboration workflows and analyzing workflow efficiency. This agent specializes in identifying bottlenecks, streamlining processes, and ensuring smooth handoffs between human creativity and AI assistance. Examples:\n\n<example>\nContext: Improving development workflow efficiency")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="WorkflowOptimizer", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = WorkflowOptimizerAgent()
    ...
    report_header = f"""
# Agent Report: WorkflowOptimizer
