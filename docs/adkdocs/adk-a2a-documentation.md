...
In this tutorial, we will explore how to build a simple multi-agent system for an e-commerce site, designed to offer the "Generative Recommendations" you find in the Shopper's Concierge demo.
...
Title: google/A2A: An open protocol enabling communication and interoperability between opaque agentic applications.
...
docs: Refactoring Python Tutorial (#474)
	


demo
	
fix: Update uv.lock files for update packages (#636)
...
chore: fix typo in types.ts doc comment (#626)
	


tests
	
chore: Run ruff/autoflake/pyupgrade formatting on all files (#411)
...
chore: add .git-blame-ignore-revs for formatting commit (#429)
	


.gitignore
	
chore: Add .vscode/settings.json for formatting (#427)
	


.prettierrc
	
ci: Add .jscpd.json for copy-paste ignore on samples (#410)
	


.ruff.toml
	
ci: Update superlinter to run Ruff (#363)
...
ci: Repository Management Init (#324)
...
docs: Python SDK Initial Doc Generation (#570)
...
docs: Fix Broken links (#522)
	


mkdocs.yml
	
docs: Update SDK Doc to show all enum values (#637)
...
Title: What is A2A? - Agent2Agent Protocol (A2A)
...
Next, learn about the Key Concepts that form the foundation of the A2A protocol.
...
Title: Agent Discovery - Agent2Agent Protocol (A2A)
...
Title: A2A and MCP - Agent2Agent Protocol (A2A)
...
Framework Bugs: Open an issue in google/adk-python or in google/adk-java
...
Framework Enhancements: Open an issue in google/adk-python or in google/adk-java
...
Python Framework: Create a Pull Request in google/adk-python
Java Framework: Create a Pull Request in google/adk-java
...
Title: Introduction - Agent2Agent Protocol (A2A)
...
Title: Setup - Agent2Agent Protocol (A2A)
...
Title: Key Concepts - Agent2Agent Protocol (A2A)
...
Title: Enterprise-Ready Features - Agent2Agent Protocol (A2A)
...
Title: Start Server - Agent2Agent Protocol (A2A)
...
Title: Agent Skills & Agent Card - Agent2Agent Protocol (A2A)
...
Title: Agent Executor - Agent2Agent Protocol (A2A)
...
Title: Next Steps - Agent2Agent Protocol (A2A)
...
Title: google/adk-samples: A collection of sample agents built with Agent Development (ADK)
...
Latest commit
turanbulmus
Merge pull request #130 from mstyer-google/adk-1.0-migration
bc059fc
 · 
History
162 Commits
...
Java agents readme cleanup
	


python
	
Merge branch 'main' into adk-1.0-migration
	


.gitignore
	
Sample agent cleanup
...
Title: Streaming & Asynchronous Operations - Agent2Agent Protocol (A2A)
...
Event Structure: The server sends events over this stream. Each event's data field contains a JSON-RPC 2.0 Response object, specifically a SendStreamingMessageResponse. The id in this JSON-RPC response matches the id from the client's original message/stream request.
Event Types (within SendStreamingMessageResponse.result):
Task: Represents the stateful unit of work being processed by the A2A Server for an A2A Client.
...
TaskArtifactUpdateEvent: Delivers new or updated Artifacts generated by the task. This is used to stream large files or data structures in chunks. This object itself contains fields like append, and lastChunk to help the client reassemble the complete artifact.
...
Separately, using the tasks/pushNotificationConfig/set RPC method for an existing task. The PushNotificationConfig includes:
...
Title: Streaming & Multiturn - Agent2Agent Protocol (A2A)
...
Title: Specification - Agent2Agent Protocol (A2A)
...
| Field Name | Type | Required | Description | | :--------- | :-------------------- | :---------- | :-------------------------------------------- | --------------------------------------------- | | kind | "file" (literal) | Yes | Identifies this part as file content. | | file | FileWithBytes | FileWithUri | Yes | Contains the file details and data/reference. | | metadata | Record<string, any> | No | Optional metadata specific to this file part. |
...
authentication	AuthenticationInfo	No	Authentication details the A2A Server must use when calling the url. The client's webhook (receiver) defines these requirements.
6.9. PushNotificationAuthenticationInfo Object¶
...
OR TaskArtifactUpdateEvent	Yes	The event payload
7.2.2. TaskStatusUpdateEvent Object¶
...
Client message/send (initial request):
{
...
Client message/send, Part.metadata hints at desired output schema/MIME type: (Note: A2A doesn't formally standardize schema negotiation in v0.2.0, but metadata can be used for such hints by convention between client/server).
{
...
Streaming in ADK adds the low-latency bidirectional voice and video interaction capability of Gemini Live API to AI agents.
