...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("seo_content_writer_sysp.json")

logger = logging.getLogger("SeoContentWriter")
...
class SeoContentWriterAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Writes SEO-optimized content based on provided keywords and topic briefs. Creates engaging, comprehensive content following best practices. Use PROACTIVELY for content creation tasks.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for SeoContentWriter...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Writes SEO-optimized content based on provided keywords and topic briefs. Creates engaging, comprehensive content following best practices. Use PROACTIVELY for content creation tasks.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="SeoContentWriter", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = SeoContentWriterAgent()
