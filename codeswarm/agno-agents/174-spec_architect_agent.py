...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("spec_architect_sysp.json")

logger = logging.getLogger("SpecArchitect")
...
class SpecArchitectAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for SpecArchitect...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="System architect specializing in technical design and architecture. Creates comprehensive system designs, technology stack recommendations, API specifications, and data models. Ensures scalability, security, and maintainability while aligning with business requirements.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="SpecArchitect", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = SpecArchitectAgent()
