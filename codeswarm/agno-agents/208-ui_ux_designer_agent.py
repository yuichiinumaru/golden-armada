...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("ui_ux_designer_sysp.json")

logger = logging.getLogger("UiUxDesigner")
...
class UiUxDesignerAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for UiUxDesigner...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Create interface designs, wireframes, and design systems. Masters user research, accessibility standards, and modern design tools. Specializes in design tokens, component libraries, and inclusive design. Use PROACTIVELY for design systems, user flows, or interface optimization.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="UiUxDesigner", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = UiUxDesignerAgent()
