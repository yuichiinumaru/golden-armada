...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("context_manager_sysp.json")
...
class ContextManagerAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert context manager specializing in information storage, retrieval, and synchronization across multi-agent systems. Masters state management, version control, and data lifecycle with focus on ensuring consistency, accessibility, and performance at scale.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for ContextManager...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert context manager specializing in information storage, retrieval, and synchronization across multi-agent systems. Masters state management, version control, and data lifecycle with focus on ensuring consistency, accessibility, and performance at scale.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="ContextManager", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = ContextManagerAgent()
