...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("react_nextjs_expert_sysp.json")

logger = logging.getLogger("ReactNextjsExpert")
...
class ReactNextjsExpertAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert in Next.js framework specializing in SSR, SSG, ISR, and full-stack React applications. Provides intelligent, project-aware Next.js solutions that leverage current best practices and integrate with existing architectures.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for ReactNextjsExpert...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert in Next.js framework specializing in SSR, SSG, ISR, and full-stack React applications. Provides intelligent, project-aware Next.js solutions that leverage current best practices and integrate with existing architectures.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="ReactNextjsExpert", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = ReactNextjsExpertAgent()
    ...
    report_header = f"""
# Agent Report: ReactNextjsExpert
