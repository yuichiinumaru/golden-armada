...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("multi_agent_coordinator_sysp.json")
...
class MultiAgentCoordinatorAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for MultiAgentCoordinator...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert multi-agent coordinator specializing in complex workflow orchestration, inter-agent communication, and distributed system coordination. Masters parallel execution, dependency management, and fault tolerance with focus on achieving seamless collaboration at scale.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="MultiAgentCoordinator", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = MultiAgentCoordinatorAgent()
    ...
    report_header = f"""
# Agent Report: MultiAgentCoordinator
