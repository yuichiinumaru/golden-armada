...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("seo_cannibalization_detector_sysp.json")

logger = logging.getLogger("SeoCannibalizationDetector")
...
class SeoCannibalizationDetectorAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Analyzes multiple provided pages to identify keyword overlap and potential cannibalization issues. Suggests differentiation strategies. Use PROACTIVELY when reviewing similar content.")
        ...
        self.agent = Agent(
            model=self.model,
            tools=tools_list,
            name="SeoCannibalizationDetector",
            description=description,
            instructions=instructions,
            markdown=True
        )
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for SeoCannibalizationDetector...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Analyzes multiple provided pages to identify keyword overlap and potential cannibalization issues. Suggests differentiation strategies. Use PROACTIVELY when reviewing similar content.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="SeoCannibalizationDetector", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = SeoCannibalizationDetectorAgent()
    ...
    report_header = f"""
# Agent Report: SeoCannibalizationDetector
