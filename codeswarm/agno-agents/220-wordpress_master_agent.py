...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("wordpress_master_sysp.json")

logger = logging.getLogger("WordpressMaster")
...
class WordpressMasterAgent:
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for WordpressMaster...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Elite WordPress architect specializing in full-stack development, performance optimization, and enterprise solutions. Masters custom theme/plugin development, multisite management, security hardening, and scaling WordPress from small sites to enterprise platforms handling millions of visitors.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="WordpressMaster", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = WordpressMasterAgent()
