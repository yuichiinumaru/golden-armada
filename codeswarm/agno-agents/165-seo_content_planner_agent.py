...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("seo_content_planner_sysp.json")

logger = logging.getLogger("SeoContentPlanner")
...
class SeoContentPlannerAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Creates comprehensive content outlines and topic clusters for SEO. Plans content calendars and identifies topic gaps. Use PROACTIVELY for content strategy and planning.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for SeoContentPlanner...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Creates comprehensive content outlines and topic clusters for SEO. Plans content calendars and identifies topic gaps. Use PROACTIVELY for content strategy and planning.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="SeoContentPlanner", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = SeoContentPlannerAgent()
    ...
    report_header = f"""
# Agent Report: SeoContentPlanner
