...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("project_analyst_sysp.json")

logger = logging.getLogger("ProjectAnalyst")
...
class ProjectAnalystAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "MUST BE USED to analyse any new or unfamiliar codebase. Use PROACTIVELY to detect frameworks, tech stacks, and architecture so specialists can be routed correctly.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for ProjectAnalyst...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="MUST BE USED to analyse any new or unfamiliar codebase. Use PROACTIVELY to detect frameworks, tech stacks, and architecture so specialists can be routed correctly.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="ProjectAnalyst", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = ProjectAnalystAgent()
