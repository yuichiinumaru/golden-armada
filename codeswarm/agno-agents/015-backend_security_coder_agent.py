...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("backend_security_coder_sysp.json")

logger = logging.getLogger("BackendSecurityCoder")
...
class BackendSecurityCoderAgent:
    def __init__(self):
        self.model = None
        self.agent = None
        self.mcp_tools = None
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert in secure backend coding practices specializing in input validation, authentication, and API security. Use PROACTIVELY for backend security implementations or security code reviews.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for BackendSecurityCoder...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert in secure backend coding practices specializing in input validation, authentication, and API security. Use PROACTIVELY for backend security implementations or security code reviews.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="BackendSecurityCoder", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = BackendSecurityCoderAgent()
    ...
    report_header = f"""
# Agent Report: BackendSecurityCoder
