...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("django_orm_expert_sysp.json")

logger = logging.getLogger("DjangoOrmExpert")
...
class DjangoOrmExpertAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "Expert in Django ORM optimisation, complex queries, and database performance. Masters query optimisation, database design, and migrations for high-performance Django applications while respecting existing project architecture.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for DjangoOrmExpert...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="Expert in Django ORM optimisation, complex queries, and database performance. Masters query optimisation, database design, and migrations for high-performance Django applications while respecting existing project architecture.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="DjangoOrmExpert", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = DjangoOrmExpertAgent()
