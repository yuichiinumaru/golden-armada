...
SYSTEM_PROMPT_FILE = Path(__file__).with_name("fullstack_developer_sysp.json")

logger = logging.getLogger("FullstackDeveloper")
...
class FullstackDeveloperAgent:
    ...
    async def setup_agent(self):
        ...
        description = str(self.context.get("description") or "End-to-end feature owner with expertise across the entire stack. Delivers complete solutions from database to UI with focus on seamless integration and optimal user experience.")
    ...
    async def run_task(self, task: str) -> str:
        ...
        print(f"🚀 Starting task for FullstackDeveloper...")
        print(f"📝 Task: {task}")
        print("-" * 70 + "\n")
...
async def main():
    parser = argparse.ArgumentParser(description="End-to-end feature owner with expertise across the entire stack. Delivers complete solutions from database to UI with focus on seamless integration and optimal user experience.")
    parser.add_argument("--task", required=True, help="The task for the agent to perform.")
    parser.add_argument("--timeout", type=int, default=600, help="Timeout in seconds.")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging.")
    args = parser.parse_args()

    log_file = setup_logging(log_filename_prefix="FullstackDeveloper", verbose=args.verbose)
    print(f"🗒️  Log saved to: {log_file}")
    
    agent_instance = FullstackDeveloperAgent()
    ...
    report_header = f"""
# Agent Report: FullstackDeveloper
