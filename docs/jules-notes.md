...
*   **API Model Compatibility:**
    *   The underlying API (likely `v1beta` for ADK 1.1.1) has specific model compatibilities. For instance, `gemini-pro` (or older model versions) sometimes resulted in 404 (Not Found) errors. Switching to `gemini-2.5-flash-preview-05-20` (or simply `gemini-1.5-flash`) resolved these API access issues.
